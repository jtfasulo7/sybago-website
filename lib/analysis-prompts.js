 ...
 *
 * WHY A TOOL CALL, NOT PROSE. The panels render structured text with a styled
 * callout, and parsing markdown back into that shape is guesswork. Asking for a
 * bare JSON object instead only moved the problem: a preamble, a code fence or a
 * thinking block all arrive as an unparseable answer, and both panels failed
 * that way. A forced report_analysis tool call has no envelope to get wrong, and
 * it kills the chat register outright — there is nowhere for "Sure! Let me
 * analyse..." to go.
 */
 ...
 *
 *  The SHAPE of the answer is enforced by the `report_analysis` tool schema in
 *  api/ads-analysis.js, not by asking for JSON here. This section covers what
 *  goes IN each field — length, register, and the things that make an analysis
 *  worth reading. */
const OUTPUT_CONTRACT = `
OUTPUT

Answer by calling the \`report_analysis\` tool. That is the only way to reply —
there is no prose response, and nothing you write outside the tool call is
shown to anyone.
...
Fill it in like this:
  ...
  finding         One or two short paragraphs. The single most likely problem,
                  or confirmation that performance is healthy. Separate
                  paragraphs with a blank line.
  recommendation  One short paragraph. The specific next action.
  flag            OMIT unless a KPI is genuinely at a critical threshold.

When a flag is warranted, its level means:
...
Most analyses should carry no flag at all. One that fires on every reading
stops being read.
