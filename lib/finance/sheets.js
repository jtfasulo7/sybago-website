...
import { normaliseCost } from './config.js';
...
export function seedEnteredRows(manual) {
  ...
  // Monthly, matching the rest of the sheet — the "Monthly" column would
  // otherwise carry a yearly figure for anything billed annually.
  for (const c of manual.recurringCosts) {
    const n = normaliseCost(c);
    rows.push(['Expense', n.label, n.amount, [n.paidBy ? `Paid by ${n.paidBy}.` : '', n.note || ''].filter(Boolean).join(' ')]);
  }
