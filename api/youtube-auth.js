// The Google OAuth round trip for YouTube uploads.
//
//   GET /api/youtube-auth            -> redirects to Google to authorise
//   GET /api/youtube-auth?code=...   -> Google sends the browser back here
//   GET /api/youtube-auth?status=1   -> JSON: connected or not, and why
//
// This URL is what goes in the "Authorised redirect URIs" list of the OAuth
// client, on a client of type **Web application**. One endpoint for both legs,
// because Google matches redirect_uri byte for byte on the token exchange
// against the one used to request the code — two URLs is two chances for them
// to disagree, and the error is a bare redirect_uri_mismatch.
...
import {
  ...
  youtubeConfig,
  youtubeStatus,
} from '../lib/social/youtube-auth.js';

const STATE_COOKIE = 'sybago_yt_state';
 ...
 * YouTube channel to this dashboard's stored credentials. The cookie proves the
...
function issueState(res, secret) {
  ...
  res.setHeader('Set-Cookie', [
    `${STATE_COOKIE}=${state}; Path=/api/youtube-auth; HttpOnly; Secure; SameSite=Lax; Max-Age=600`,
  ]);
  return state;
}
...
function clearState(res) {
  res.setHeader('Set-Cookie', [
    `${STATE_COOKIE}=; Path=/api/youtube-auth; HttpOnly; Secure; SameSite=Lax; Max-Age=0`,
  ]);
}
...
export default async function handler(req, res) {
  ...
  if (q.status) {
    ...
    try {
      return res.status(200).json(await youtubeStatus());
    } catch (e) {
  ...
  if (q.code || q.error) {
    if (q.error) {
      clearState(res);
      /* `access_denied` is a person clicking Cancel, or clicking away from the
         "Google hasn't verified this app" screen rather than through Advanced.
         Worth naming, because it is the most likely thing to happen here and it
         is not a fault. */
      const why = String(q.error) === 'access_denied'
        ? '<p>The authorisation was declined. If you saw a “Google hasn’t verified this app” ' +
          'screen, choose <b>Advanced</b> and then <b>Go to Sybago</b> — the warning is expected ' +
          'for an app that has not been through Google’s review, and nothing was saved.</p>'
        : `<p>Google returned <code>${escape(q.error)}</code>. Nothing was saved.</p>`;
      return page(res, 400, 'YouTube did not authorise', why);
    }

    if (!stateMatches(req, q.state, secret)) {
      clearState(res);
      // Refusing here is the whole point of state: without it a link from
      // anywhere could bind someone else's channel to this dashboard.
    ...
    try {
      ...
      res.status(302).setHeader('Location', '/dashboard?youtube=connected');
      return res.end();
    } catch (e) {
      clearState(res);
      return page(res, 502, 'Google would not issue a token',
  ...
  // session-gated the same way — Google's redirect is a fresh top-level
  ...
  if (!cfg) {
    return res.status(500).json({
      ...
      message: `Set ${missingYoutubeEnv().join(' and ')} from the Google Cloud OAuth client, then try again.`,
    });
  }
  if (!secret) {
    return res.status(500).json({
  ...
  res.status(302).setHeader('Location', authUrl({ clientId: cfg.clientId, redirect, state }));
  res.end();
}
